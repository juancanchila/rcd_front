...
createPredio(): FormGroup {
  ...
  getLicenciaOptions(blockName: string): string[] {
    ...
    return selectedOptions.map((key) => {
      ...
      if (key === 'otra_construccion') {
        const otraCual = this.proyecto.get(
          'tipo_licencia_0100.construccion_block_0100.otra_cual_construccion_0100'
        )?.value;
        return otraCual ? `Otra: ${otraCual}` : 'Otra (sin especificar)';
      }
      if (key === 'otra_espacio_publico') {
        const otraCual = this.proyecto.get(
          'tipo_licencia_0100.espacio_publico_block_0100.otra_cual_espacio_publico_0100'
        )?.value;
        return otraCual ? `OTRA: ${otraCual}` : 'OTRA (sin especificar)';
      }
  ...
  getDemolicionRespuesta(): string | null {
    return (
      this.proyecto.get('tipo_licencia_0100.demolicion_ruina_block_0100.demolicion_respuesta_0100')
        ?.value || null
    );
  }
